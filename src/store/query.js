...
function patch_wke_property(state, { obj, scope, active_wke_id }) {
    ...
    state.worksheets_arr = scope.vue.$helper.immutableUpdate(state.worksheets_arr, {
...
export default {
    ...
    mutations: {
        ...
        UPDATE_WKE(state, { idx, wke }) {
            state.worksheets_arr = this.vue.$helper.immutableUpdate(state.worksheets_arr, {
                [idx]: { $set: wke },
            })
        },
    ...
    actions: {
        ...
        async disconnectAll({ state, dispatch }) {
            try {
                const cnctResources = this.vue.$helper.lodash.cloneDeep(state.cnct_resources)
        ...
        deleteInvalidConn({ dispatch }, invalidCnctResources) {
            try {
                invalidCnctResources.forEach(id => {
                    this.vue.$helper.deleteCookie(`conn_id_body_${id}`)
        ...
        async reloadTreeNodes({ commit, dispatch, state }) {
            const active_wke_id = state.active_wke_id
            const expanded_nodes = this.vue.$helper.lodash.cloneDeep(state.expanded_nodes)
            await dispatch('queryingActionWrapper', {
                action: async () => {
                    commit('UPDATE_DB_TREE_MAP', {
                        id: active_wke_id,
        ...
        async fetchPrvw({ state, rootState, commit, dispatch }, { tblId, prvwMode }) {
            ...
            await dispatch('queryingActionWrapper', {
                action: async () => {
                    ...
                    let sql
                    const escapedTblId = this.vue.$helper.escapeIdentifiers(tblId)
                    switch (prvwMode) {
        ...
        changeWkeName({ state, getters, commit }, name) {
            let newWke = this.vue.$helper.lodash.cloneDeep(getters.getActiveWke)
            newWke.name = name
            commit('UPDATE_WKE', {
