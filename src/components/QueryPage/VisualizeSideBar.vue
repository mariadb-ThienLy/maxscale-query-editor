    ...
    methods: {
        addResultSetsWatcher() {
            ...
            this.rmResultSetsWatcher = this.$watch('resultSets', (v, oV) => {
                if (!this.$helper.lodash.isEqual(v, oV)) {
                    this.clearAxisVal()
                    this.resSet = null
                    this.genChartData({ axis: this.axis, chartType: this.selectedChart })
                }
        ...
        genDataset({ colorIndex, data, chartType }) {
            const lineColor = this.$helper.dynamicColors(colorIndex)
            let dataset = {
                data,
            }
            const indexOfOpacity = lineColor.lastIndexOf(')') - 1
            const backgroundColor = this.$helper.strReplaceAt({
                str: lineColor,
        ...
        genChartData({ axis, chartType }) {
            ...
            if (axis.x && axis.y) {
                ...
                let dataPoints = []
                let labels = []
                const dataRows = this.$helper.getObjectRows({
