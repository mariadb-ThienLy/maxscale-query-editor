    ...
    watch: {
        ...
        showVisChart(v) {
            if (v) {
                ...
                this.minQueryPanePct = this.$helper.pxToPct({
                    px: 32,
    ...
    methods: {
        ...
        handleSetSidebarPct() {
            ...
            if (this.is_sidebar_collapsed) {
                this.minSidebarPct = this.$helper.pxToPct({ px: 40, containerPx: containerWidth })
                this.sidebarPct = this.minSidebarPct
            } else {
                this.minSidebarPct = this.$helper.pxToPct({ px: 200, containerPx: containerWidth })
                this.sidebarPct = this.$helper.pxToPct({ px: 240, containerPx: containerWidth })
            }
        },
        handleSetMinEditorPct() {
            this.minEditorPct = this.$helper.pxToPct({ px: 26, containerPx: this.ctrDim.height })
        },
        setResultPaneDim() {
        ...
        handleSetVisSidebar(showVisSidebar) {
            if (showVisSidebar) {
                const visSidebarPct = this.$helper.pxToPct({
                    px: 250,
        ...
        dropTxtToEditor({ e, type }) {
            if (e.target.textContent) {
                ...
                if (dropTarget) {
                    ...
                    switch (type) {
                        case 'schema':
                            text = this.$helper.escapeIdentifiers(text)
                            break
                    }
