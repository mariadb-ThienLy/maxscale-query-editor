<template>
    <v-select
        ...
        :placeholder="
            showPlaceHolder ? `select ${multiple ? 2 : 1} ${entityName} ${multiple ? 2 : 1}` : ''
        "
        :no-data-text="'noEntityAvailable'"
...
export default {
    ...
    computed: {
        ...
        hasChanged: function() {
            let isEqual = this.$helper.lodash.isEqual(this.selectedItems, this.defaultItems)
            return !isEqual
        },
        selectedItems: {
            get() {
